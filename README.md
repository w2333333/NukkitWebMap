...
[English](#-features) | [中文](#-功能特性) | [更新日志](#-changelog--更新日志)

</div>

---

## 📢 What's New in v1.1.0 / 更新内容

### 🆕 New Features / 新功能

| Feature | Description | 说明 |
|:-------:|-------------|------|
| 💾 | **Wall Persistence** | Map walls are now saved and automatically restored after server restart |
| 💾 | **地图墙持久化** | 地图墙现在会保存，服务器重启后自动恢复 |
| 🎨 | **Web-Matched Markers** | Player markers now match web interface proportions |
| 🎨 | **网页同比例标记** | 玩家标记现在与网页界面保持相同比例 |
| 🗑️ | **Remove Wall Command** | New `/webmap removewall` command to delete wall data |
| 🗑️ | **删除地图墙命令** | 新增 `/webmap removewall` 命令删除地图墙数据 |

### 🔧 Improvements / 改进

- ✅ Fixed map wall display issues after server restart / 修复服务器重启后地图墙显示问题
- ✅ Player marker size scales with wall size / 玩家标记大小随地图墙尺寸缩放
- ✅ Added name tag background box (like web) / 添加名字标签背景框（与网页一致）
- ✅ Local JAR compilation support / 支持本地 JAR 编译
...
| `/webmap removewall` | Remove map wall data | 删除地图墙数据 |
...
> 💾 **Persistence / 持久化**: Map walls are automatically saved and restored after server restart!
> 
> 地图墙会自动保存，重启服务器后自动恢复！
...
**🆕 New Features / 新功能:**
- 💾 **Wall Persistence** - Map walls saved to `walls.yml`, auto-restore after restart
- 💾 **地图墙持久化** - 地图墙保存到 `walls.yml`，重启后自动恢复
- 🎨 **Proportional Markers** - Player markers match web interface (2.5% dot, 2.2% font)
- 🎨 **比例标记** - 玩家标记与网页界面一致（2.5%标点，2.2%字体）
- 🗑️ **Remove Wall** - New `/webmap removewall` command
- 🗑️ **删除地图墙** - 新增 `/webmap removewall` 命令
- 📦 **Local Build** - Support local JAR compilation (no network needed)
- 📦 **本地编译** - 支持本地 JAR 编译（无需网络）

**🔧 Fixes / 修复:**
- Fixed wall display after server restart / 修复重启后地图墙显示问题
- Fixed marker size on large walls / 修复大尺寸地图墙标记大小问题

### v1.0.0

- 🌐 Initial release / 首次发布
- Web map with player tracking / 网页地图玩家追踪
- In-game map wall / 游戏内地图墙
- Region limit / 区域限制
