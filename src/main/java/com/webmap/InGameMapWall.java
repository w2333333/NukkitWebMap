...
public class InGameMapWall {
    ...
    // Cached base images (avoid disk reads)
    ...
    private void updateWallsAsync() {
        ...
        plugin.getServer().getScheduler().scheduleAsyncTask(plugin, new AsyncTask() {
            ...
            public void onRun() {
                for (Map.Entry<String, WallInfo> entry : physicalWalls.entrySet()) {
                    ...
                    try {
                        ...
                        // Web: dot=16px, font=13px, stroke=2px on ~600px display
                        // Ratio: dot=2.5%, font=2.2%, stroke=0.35%
                        ...
                        for (PlayerPos p : players) {
                            ...
                            if (relX >= 0 && relX <= 1 && relZ >= 0 && relZ <= 1) {
                                ...
                                // Draw red dot with white border (like website)
                                g.setColor(new Color(255, 51, 51)); // #ff3333
                                ...
                                // Draw name with background box (like website .tag)
                                String name = p.name;
                                FontMetrics fm = g.getFontMetrics();
                                int textWidth = fm.stringWidth(name);
                                int textHeight = fm.getAscent();
                                
                                // Position name above dot
                                ...
                                // Draw background box
                                g.setColor(new Color(0, 0, 0, 230)); // rgba(0,0,0,0.9)
            ...
            public void onCompletion(Server server) {
                ...
                // Batch send packets over multiple ticks
    ...
    /**
     * Send packets in small batches over multiple ticks
     */
    private void sendPacketsInBatches(List<PacketData> packets) {
        final int BATCH_SIZE = 10; // Send 10 packets per tick
        final int[] index = {0};
